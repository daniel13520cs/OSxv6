/* Author: chih-chien Hsiao
Comment : When the number of keys are 300000, 4 threads using
spinlock requires 10 seceonds more than that of the mutex lock takes.
This is not as I expected because mutex lock sleeps when the critical section
is locked and switch to another thread. This should be faster
than spinlock polling the other thread to wait for the resource become available.
But, after carefully thought, I figured out that if executing the critical
section is faster than putting a thread to sleep and wake up all the threads
waiting for entering the critical section, than spinlock 
would be faster than mutex lock because next thread is scheduled to 
enter the critical section faster by spinlock.*/
